...
# 01: Load driver pair table ----
{
  batch_driver_source <- NA_character_

  if (file.exists(summary_csv)) {
    batch_driver_source <- "summary_csv"
    ...
    batch_driver_source <- "eligible_pairs_csv"
    ...
    batch_driver_source <- "directory_scan"
  ...
  summary_dt[, batch_driver_source := batch_driver_source]
  print(summary_dt)
}

# 02: Annotate pair outputs ----
{
  ...
  for (i in seq_len(nrow(summary_dt))) {
    ...
    batch_results[[i]] <- data.table::data.table(
      ...
      batch_driver_source = summary_row$batch_driver_source[[1]],
...
# 03: Write annotation summary ----
{
  ...
  print(
    annotation_dt[
      ,
      .N,
      by = .(batch_driver_source, annotation_status)
    ][order(batch_driver_source, annotation_status)]
  )
}
